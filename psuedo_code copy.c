...
char *add_leftover(char *buffer, char *leftover_str)
{
    // Bu fonksiyon, bir önceki satır okumasındaki buffer'da '\n'
    // karakterinden sonra artık karakter kaldıysa bunu temp'e ekler
    char *temp;
    int i = 0;

    if (!leftover_str || leftover_str[0] == '\0') {
        return NULL; // Sayfa başı veya artık veri kalmadı
    }

    temp = (char *)malloc(sizeof(char) * (ft_strlen(leftover_str) + 1));
    while (leftover_str[i]) {
        temp[i] = leftover_str[i];
        i++;
    }
    temp[i] = '\0'; // String sonunu işaretle

    return temp;
}

char *merge_str(buffer)
{
	// Bu fonksiyon satır sonuna gelene kadar 
	char	*temp;
	int		i;
	int		j;
	...
	temp = NULL;
	temp = add_leftover(buffer);

	if (!check_line)
	{
		temp = (char *)malloc(sizeof(char) * ft_strlen(buffer) 
				+ ft_strlen(leftover_str));
		while (buffer)
			temp[i++] = buffer++;
	}
	else
...
char	*get_next_line(int fd)
{
	...
	line = add_leftover(buffer);
	line = merge_str(buffer);
	free (leftover);
